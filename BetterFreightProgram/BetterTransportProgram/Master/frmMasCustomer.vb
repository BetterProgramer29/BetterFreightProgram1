...
#End Region
            ...
            If Userauthen.IsInsertData = 1 Then
                ...
                Else
                    If CheckCustCode(txtCustCode.Text, txtBranch.Text, "CST") Then
                        MetroFramework.MetroMessageBox.Show(Me, "Customer Code " & txtCustCode.Text & " Branch " & txtBranch.Text & " Already Existing In Database!", "Check Existing", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        Exit Sub
                    End If
                    SaveData(1)
                End If
    ...
    Private Sub LoadData()
        Try
            ...
            Else
                str &= " and CustCode='" & txtCustCode.Text & "' AND CustBranch='" & txtBranch.Text & "'"
            End If
            ...
            If dr IsNot Nothing Then
                ...
                togIsPerson.Checked = CInt(dr("IsPerson").ToString()) 'Person
                txtCustCode.ReadOnly = True
                LoadBillTo()
                LoadShipTo()
                LoadNotify()
    ...
    Private Sub ResetAllText()
        ...
        togIsPerson.Checked = False 'Person
...
and NotifyToBranch='" & txtBranch.Text & "' order by [CustCode] ", MainConnection)
    ...
    Private Sub btnSaveBank_Click(sender As Object, e As EventArgs) Handles btnSaveBank.Click
        If String.IsNullOrEmpty(txtCustCode.Text) Then Exit Sub
        ...
        End If
        'CSTBCK
        Userauthorize(UserProfile.U_Code, "CSTBCK")
        If txtIsNewBank.Text = "1" Then
            If Userauthen.IsInsertData = 1 Then
                If CheckExistCode("Mas_CustomerBook", "BookCode", txtBookCode.Text) = False Then
                    SaveDataBank(1)
                Else
    ...
    Private Sub SaveDataBank(_Insert As Integer)
        Try
            ...
            Dim nection As OleDb.OleDbConnection = New OleDb.OleDbConnection()
            nection = ConnectDB()
            Dim SQLcommand As StringBuilder = New StringBuilder("exec sp_lnsertOrupdateMas_CustomerBook ")
            ...
            SQLcommand.Append(",'" & txtCustCode.Text & "'") 'VENREF
    ...
    Private Sub LoadDataVenBank()
        Try
            ...
            nection = ConnectDB()
            Dim str As String = "SELECT * FROM Mas_CustomerBook where BranchCode='" & BrCode & "' and CMPCode='" & CMPCode & "' AND VENTBIDNT='" & HIdentity & "'"
