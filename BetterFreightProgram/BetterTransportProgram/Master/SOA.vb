...
Public Class SOa
    ...
    Private Sub SaveSoaDetail()
        Try
            ...
            For row as Integer = 0 To dgvInvoice.Rows.Count - 1
                ...
                SQLcommand.append(",'" & CInt(dgvInvoice.Rows(row).Cells(25).Value) & "'") ' SELECT
                Dim result as Integer = ExecuteNonQuery(SQLcommand.ToString(), nection)
                If result = 1 Then
                Else
                    a += 1
    ...
    Private Sub LoadDataToSoa(Optional ByVal InvoiceNo as String = "")
        Try
            ...
            Dim nection as New OleDb.OleDbConnection()
            nection = ConnectDB()
            Dim str as String = "SELECT * FROM Invoice_Header where CustomerCode='" & txtCustomerCode.Text & "' and active='1' and SoaNo IS NULL"
            da.SelectCommand = New OleDbCommand(str, nection)
